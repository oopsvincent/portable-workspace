...
import { DeletePutBackIcon } from 'hugeicons-react';
...
export default function WorkspacePage({ params }: { params: { id: string } }) {
  ...
  const [enclave, setEnclave] = useState<storage.EnclaveRecord | null>(null);
  ...
  // ===== LOAD FILES =====
  useEffect(() => {
    storage.setActiveEnclave(params.id);
    async function init() {
      if (!sessionStorage.getItem('vela-logged-in')) {
        router.push('/');
        return;
      }
      try {
        const encData = await storage.getEnclave(params.id);
        setEnclave(encData);
      ...
      <header className="h-10 border-b border-border flex items-center px-2 bg-card/30 flex-shrink-0">
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="sm" className="h-6 px-1.5 text-[10px] uppercase font-bold tracking-wider text-muted-foreground hover:text-foreground" onClick={() => router.push('/')}>
              <ChevronsLeft /> Home
            </Button>
            <div className="w-px h-3 bg-border mx-1" />
            <Package className="w-4 h-4 text-primary ml-1" />
            <span className="text-xs font-bold hidden sm:inline tracking-wide font-heading">{enclave?.name || 'Vela Enclave'}</span>
          </div>
        </div>
          ...
          {showEmptyState ? (
            <div className="flex-1 flex items-center justify-center p-8">
              <div className="text-center space-y-6 max-w-md">
                ...
                <div>
                  <h2 className="text-2xl font-bold mb-2 font-heading">{enclave?.name || 'Vela Enclave'}</h2>
